...
sin = lambda t: np.sin(1/np.random.randint(900,1500) *t)
...
def generator(x, isTrain=True,reuse=False, batch_size=batch_size):
    with tf.variable_scope('Generator', reuse=reuse):
        ...
        x =tf.layers.batch_normalization(x,momentum=0.9, training =isTrain, epsilon=0.00001)#,name = "g_bn_d1")
        x = tf.nn.relu(x)
        ...
        conv1 = tf.nn.relu(tf.layers.batch_normalization(conv1,momentum=0.9, training =isTrain,epsilon=0.00001))#,name="g_bn_conv1"))
        ...
        conv2 = tf.nn.relu(tf.layers.batch_normalization(conv2,momentum=0.9,training =isTrain,epsilon=0.00001))#,name="g_bn_conv2"))
        ...
        conv3 = tf.nn.relu(tf.layers.batch_normalization(conv3,momentum=0.9,training =isTrain,epsilon=0.00001))#, name="g_bn_conv3"))
        ...
        conv5 = tf.layers.batch_normalization(conv5,momentum=0.9,training =isTrain,epsilon=0.00001)
        out = conv5
...
def discriminator(x,isTrain=True, reuse=False):
    with tf.variable_scope('Discriminator', reuse=reuse):
        ...
        #print(x.get_shape(),'i')
        #conv1 = conv1d()
        conv1 = tf.layers.conv1d(x,4, 30,strides=3, padding = "Same",kernel_initializer = tf.contrib.layers.xavier_initializer_conv2d())
        ...
        dense1 = tf.layers.batch_normalization(dense1, momentum=0.9, training =isTrain)
        dense1 = tf.nn.leaky_relu(dense1)
        dense2 = tf.layers.dense(dense1, 512,kernel_initializer = tf.contrib.layers.xavier_initializer())
        dense2 = tf.layers.batch_normalization(dense2, momentum=0.9, training =isTrain)
        d3 = dense2
...
gen_sample, out_test = generator(random_vector, isTrain, batch_size=batch_size)
...
gen, ot  = generator(random_vector,batch_size=100,reuse=True)
...
train_gen = traing(gen_loss)
train_disc = traind(disc_loss)
"""
...
with tf.Session() as sess:
    ...
    samp, samp2 = sess.run([ot,gen], feed_dict={random_vector: z})
    
    g = sess.run(disc_real_logits, feed_dict={real_image_input: epoch_x, isTrain:False})
